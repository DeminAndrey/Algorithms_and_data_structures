...
int main() {
  vector<int> sizes = { 100, 1000, 10000, 100000, 1000000 }; // Различные размеры векторов
  ...
  for (int size : sizes) {
    ...
    vector<int> quickVec(randomVec.begin(), randomVec.end());
    vector<int> mergeVec(randomVec.begin(), randomVec.end());

    cout << "Размер вектора: " << size << '\n';

    cout << "Быстрая сортировка (QuickSort):\n";
    run(sorts::quickSort, quickVec);

    cout << "Сортировка слиянием (MergeSort):\n";
    run(sorts::mergeSort, mergeVec);
